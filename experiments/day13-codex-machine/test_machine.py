...
SPEC = importlib.util.spec_from_file_location("day13_machine", HERE / "machine.py")
...
class MachineTest(unittest.TestCase):
    ...
    def test_done_gate_rechecks_evidence_from_previous_phases(self) -> None:
        ...
        with self.assertRaisesRegex(machine.MachineError, "offline_tests"):
    ...
    def test_execution_is_scoped_to_day13(self) -> None:
        state = machine.load_json(self.state_path)
        state["phase"] = "EXECUTION"
        machine.dump_json_atomic(self.state_path, state)
        spec = machine.codex_spec(self.paths)
        self.assertEqual("workspace-write", spec["sandbox"])
        self.assertEqual(["day13/"], spec["allowed_write_paths"])
        self.assertEqual((self.root / "day13").resolve(), Path(spec["cwd"]))
