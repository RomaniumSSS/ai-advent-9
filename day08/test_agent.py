"""День 8: офлайн-проверки агента, хранилища и счёта токенов. Кредитов не тратят.

Проверки дней 6 и 7 остались целиком. Добавлены девять проверок дня 8, главные
две: `test_budget_parts_sum_to_the_whole` — разбор запроса на части не разъезжается
с самим запросом; `test_overflow_refuses_without_calling` — при переполнении окна
вызова не происходит вовсе, то есть отказ ничего не стоит.

Точность счётчика тесты не проверяют и не могут: словарь тут чужой, и «правильного»
числа, с которым сравнивать, у офлайн-теста нет. Проверяется другое — что сверка
с фактом провайдера происходит и считается верно (`test_estimate_is_compared_with_the_fact`),
а само расхождение замерено живым прогоном и записано в README.

Одна оговорка про сеть: `tiktoken` при первом в жизни обращении тянет словарь
и кладёт в свой кеш. На чистой машине первый запуск тестов потребует интернета,
дальше — нет. Кредитов это не стоит, к OpenRouter отношения не имеет.
...
from tokens import count_messages, count_text  # noqa: E402
...
def test_budget_parts_sum_to_the_whole() -> list[str]:
    """Роль + история + вопрос + разметка = ровно то, что уедет в модель.

    Проверка на разъезд сумм. Если однажды `build_messages` начнёт добавлять
    сообщение, а `budget` о нём не узнает, отчёт останется правдоподобным —
    и все выводы про «историю пора резать» будут сделаны по неверным числам.
    """
    problems = []
    agent = Agent(client=FakeClient())
    agent.ask("первый вопрос")
    budget = agent.budget("второй вопрос")
    whole = count_messages(agent.build_messages("второй вопрос"))
    if budget.prompt != whole:
        problems.append(f"части дают {budget.prompt}, целое — {whole}")
    if budget.question != count_text("второй вопрос"):
        problems.append("вопрос посчитан не как вопрос")
    if budget.system != count_text(agent.config.system_prompt):
        problems.append("роль посчитана не как роль")
    if budget.overhead <= 0:
        problems.append("разметка сообщений не посчитана вовсе")
    return problems


def test_history_is_what_grows() -> list[str]:
    """С каждым ходом дорожает история, а не вопрос. Ради этого всё и затевалось."""
    problems = []
    agent = Agent(client=FakeClient("ответ подлиннее, чем вопрос, и так каждый раз"))
    empty = agent.budget("вопрос")
    if empty.history != 0 or empty.history_share != 0:
        problems.append("у разговора без истории история чего-то весит")

    agent.ask("вопрос")
    first = agent.budget("вопрос")
    agent.ask("вопрос")
    agent.ask("вопрос")
    later = agent.budget("вопрос")
    if later.question != first.question:
        problems.append("одинаковый вопрос стал весить по-разному")
    if later.history <= first.history:
        problems.append("история не выросла за два хода")
    if later.prompt <= first.prompt:
        problems.append("вход не вырос, хотя история выросла")
    if not 0 < first.history_share < later.history_share:
        problems.append("доля истории во входе не растёт")
    return problems


def test_overflow_refuses_without_calling() -> list[str]:
    """Переполнение окна не доходит до сети: вызова нет, денег нет, следа нет."""
    problems = []
    client = FakeClient("х" * 6000)
    agent = Agent(config=AgentConfig(context_limit=4000, max_tokens=500), client=client)
    agent.ask("первый вопрос")
    calls_before = len(client.calls)
    turns_before = agent.turns

    reply = agent.ask("второй вопрос")
    if not reply.overflow:
        problems.append("переполнение не отмечено как переполнение")
    if reply.ok:
        problems.append("отказ по окну выдан за удачный ход")
    if len(client.calls) != calls_before:
        problems.append("запрос всё-таки ушёл в сеть, то есть был оплачен")
    if agent.turns != turns_before:
        problems.append("несостоявшийся ход попал в историю")
    if reply.cost is not None or reply.prompt_tokens is not None:
        problems.append("у неслучившегося вызова появилась цена")
    if reply.budget is None or reply.budget.fits:
        problems.append("в отказе нет разбора, из чего сложилось переполнение")
    return problems


def test_no_limit_means_no_guard() -> list[str]:
    """Окно не задано — проверять нечем, и агент не выдумывает ограничение.

    Обратное было бы худшим из вариантов: агент отказывал бы по числу, которое
    никто не сверял, и выглядело бы это как отказ модели.
    """
    problems = []
    client = FakeClient("х" * 40000)
    agent = Agent(client=client)
    agent.ask("первый вопрос")
    reply = agent.ask("второй вопрос")
    if reply.overflow:
        problems.append("агент отказал по окну, которого не знает")
    if len(client.calls) != 2:
        problems.append("вызов не состоялся без всякого лимита")
    if agent.budget("ещё").fits is not True:
        problems.append("без лимита бюджет обязан считаться помещающимся")
    return problems


def test_answer_ceiling_counts_toward_the_window() -> list[str]:
    """Запрос влезает, а вместе с местом под ответ — уже нет. Это отказ.

    Тот случай, который проще всего проглядеть: «промпт помещается» будет чистой
    правдой, и всё равно провайдер откажет. Окно у модели одно на вход и выход.
    """
    problems = []
    agent = Agent(
        config=AgentConfig(context_limit=1020, max_tokens=1000),
        client=FakeClient(),
    )
    budget = agent.budget("короткий вопрос")
    if budget.prompt >= budget.limit:
        problems.append("тест собран неверно: вход и сам по себе не влезает")
    if budget.fits:
        problems.append("бюджет не учёл место под ответ")
    reply = agent.ask("короткий вопрос")
    if not reply.overflow:
        problems.append("агент отправил запрос, которому некуда отвечать")
    return problems


def test_window_smaller_than_answer_is_a_typo() -> list[str]:
    """Окно не больше потолка ответа — опечатка, и падать надо сразу."""
    problems = []
    try:
        AgentConfig(context_limit=500, max_tokens=500)
        problems.append("окно размером с потолок ответа принято молча")
    except ValueError:
        pass
    try:
        AgentConfig(context_limit=0)
        problems.append("нулевое окно принято молча")
    except ValueError:
        pass
    # А законное сочетание обязано проходить: проверка не должна быть строже цели.
    AgentConfig(context_limit=8192, max_tokens=1000)
    return problems


def test_estimate_is_compared_with_the_fact() -> list[str]:
    """Оценка живёт рядом с фактом провайдера, и расхождение считается.

    Смысл проверки не в точности словаря — она заведомо приблизительная, — а в
    том, что сверка вообще происходит. Счётчик без сверки врёт незаметно.
    """
    problems = []
    client = FakeClient(usage=SimpleNamespace(prompt_tokens=200, completion_tokens=20))
    agent = Agent(client=client)
    reply = agent.ask("вопрос")
    if reply.budget is None:
        problems.append("оценка не доехала до ответа, сверять нечем")
        return problems
    if reply.drift is None:
        problems.append("расхождение не посчитано при известном факте")
    else:
        expected = (reply.budget.prompt - 200) / 200 * 100
        if abs(reply.drift - expected) > 1e-9:
            problems.append(f"расхождение посчитано неверно: {reply.drift}")

    # Провайдер не вернул usage — сверять не с чем, и ноль здесь соврал бы про
    # идеальное попадание, то есть выдал бы отсутствие проверки за её успех.
    blind = Agent(
        client=FakeClient(
            usage=SimpleNamespace(prompt_tokens=None, completion_tokens=None)
        )
    )
    if blind.ask("вопрос").drift is not None:
        problems.append("расхождение посчитано без факта, с которым сверяться")
    return problems


def test_overflow_leaves_the_disk_clean() -> list[str]:
    """Отказ по окну ничего не пишет в базу и не ломает прошлый разговор."""
    problems = []
    temp = TempStore("окно")
    store = temp.open()
    agent = Agent(
        config=AgentConfig(context_limit=4000, max_tokens=500),
        client=FakeClient("х" * 6000),
        store=store,
    )
    agent.ask("первый вопрос")
    ...
    if after["turns"] != before["turns"]:
        problems.append("несостоявшийся ход записан в базу")
    if len(store.load()) != len(agent.history):
        problems.append("память и диск разъехались после отказа")

    # И главное: разговор не испорчен, следующий агент поднимет ровно то,
    # что было до отказа.
    again = Agent(
        config=AgentConfig(context_limit=4000, max_tokens=500),
        client=FakeClient(),
    ...
    if again.turns != 1:
        problems.append(f"после отказа восстановилось {again.turns} ходов вместо 1")
    return problems


def test_base_counts_tokens_not_only_money() -> list[str]:
    """Суммы токенов считает база, и таблица роста идёт по порядку ходов."""
    problems = []
    # Ссылку на TempStore надо держать: каталог живёт, пока живёт объект, и
    # `TempStore(...).open()` одной строкой снёс бы базу прямо под тестом.
    temp = TempStore("рост")
    store = temp.open()
    ...
    agent.ask("третий")
    stats = store.stats()
    if stats["prompt_tokens"] != 300 or stats["completion_tokens"] != 60:
        problems.append(f"суммы токенов посчитаны неверно: {stats}")
    rows = store.growth()
    if len(rows) != 3:
        problems.append(f"в таблице роста {len(rows)} строк вместо 3")
    if any(row["prompt_tokens"] is None for row in rows):
        problems.append("в таблице роста есть ход без замеров")
    if [row["created_at"] for row in rows] != sorted(row["created_at"] for row in rows):
        problems.append("таблица роста идёт не по порядку ходов")
    return problems
...
TESTS = (
    ...
    test_budget_parts_sum_to_the_whole,
    test_history_is_what_grows,
    test_overflow_refuses_without_calling,
    test_no_limit_means_no_guard,
    test_answer_ceiling_counts_toward_the_window,
    test_window_smaller_than_answer_is_a_typo,
    test_estimate_is_compared_with_the_fact,
    test_overflow_leaves_the_disk_clean,
    test_base_counts_tokens_not_only_money,
)
