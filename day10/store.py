"""День 9: архив, summary и журнал расхода в SQLite.

Схема v3 добавляет вид вызова и отдельно сохраняет summary с границей архива.
База дня 9 самостоятельна; каждый завершённый ход записывается транзакцией."""
...
class SqliteStore:
            ...
            with connection:
                connection.execute(
                    ...
                    (
                        ...
                        reply.completion_tokens, reply.cost, reply.cost_reported, reply.empty, kind,
                    ),
                )
    ...
    def clear(self) -> None:
        ...
        with self._connect() as connection:
            with connection:
                ...
                connection.execute("DELETE FROM compression WHERE session = ?", (self.session,))
                connection.execute("DELETE FROM web_requests WHERE session = ?", (self.session,))

    def load_compression(self):
        with self._connect() as connection:
            row = connection.execute(
                "SELECT summary, covered FROM compression WHERE session = ?", (self.session,)
            ).fetchone()
        return dict(row) if row else {"summary": "", "covered": 0}

    def save_compression(self, summary, covered):
        with self._connect() as connection:
            with connection:
                count = connection.execute(
                    "SELECT COUNT(*) FROM messages WHERE session = ?", (self.session,)
                ).fetchone()[0]
                if covered > count or covered < 0 or covered % 2:
                    raise ValueError("граница summary не соответствует сохранённой истории")
                connection.execute(
                    "INSERT INTO compression VALUES (?, ?, ?, ?) ON CONFLICT(session) "
                    "DO UPDATE SET summary=excluded.summary, covered=excluded.covered, "
                    "updated_at=excluded.updated_at",
                    (self.session, summary, covered, now()),
                )

    def usage_by_kind(self):
        calls = self.growth()
        result = {}
        for kind in ("chat", "summary", "total"):
            ...
            entry["tokens"] = (entry["prompt_tokens"] + entry["completion_tokens"]
                               if entry["prompt_tokens"] is not None and entry["completion_tokens"] is not None else None)
            result[kind] = entry
        return result
            ...
            with connection:
                ...
                if row:
                    if row["question"] != question:
                        raise ValueError("request_id уже использован для другого вопроса")
                    if row["response"] is None:
                        raise RuntimeError("запрос ещё выполняется или прерван; проверьте историю перед новым запросом")
                    return json.loads(row["response"])
                connection.execute("INSERT INTO web_requests VALUES (?, ?, ?, NULL)",
    ...
    def finish_request(self, request_id, response):
        with self._connect() as connection:
            with connection:
                connection.execute("UPDATE web_requests SET response=? WHERE session=? AND request_id=?",
                                   (json.dumps(response, ensure_ascii=False), self.session, request_id))
