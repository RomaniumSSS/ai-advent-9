...
# Первые слова FACTS_SYSTEM_PROMPT из facts.py. Импортировать оттуда нельзя:
# бюджет поднимается и без агента — его зовут отдельные скрипты сверки.
FACTS_KIND_MARKER = "Ты — модуль памяти диалога"
...
class BudgetLedger:
    def __init__(self, path=DEFAULT_LEDGER, limit=0.50):
        ...
        with self.connect() as db:
            ...
            if db.execute("SELECT ceiling FROM settings").fetchone()[0] != limit:
                raise ValueError(
                    "нельзя незаметно изменить бюджет существующего эксперимента"
                )
        self.limit = limit
    ...
    def reserve(self, amount, label):
        ...
        with self.connect() as db:
            ...
            rows = db.execute(
                "SELECT reserved, actual, state FROM reservations"
            ).fetchall()
            ...
            spent = sum(
                ...
                for reserved, actual, _ in rows
            )
            if spent + amount > self.limit:
                settled_or_unknown = sum(
                    actual if actual is not None else reserved
                    for reserved, actual, state in rows
                    if state != "pending"
                )
                if (
                    any(state == "pending" for _, _, state in rows)
                    and settled_or_unknown + amount <= self.limit
                ):
                    raise BudgetBusy(
                        "деньги зарезервированы другими выполняющимися вызовами"
                    )
                raise RuntimeError(
                    f"бюджет: осталось ${self.limit - spent:.4f}, требуется резерв ${amount:.4f}"
                )
            return db.execute(
                "INSERT INTO reservations(label,reserved,state) VALUES (?, ?, ?)",
    ...
    def settle(self, ticket, actual, response_id=None):
        ...
        with self.connect() as db:
            reserved = db.execute(
                "SELECT reserved FROM reservations WHERE id=?", (ticket,)
            ).fetchone()[0]
            state = (
                "unknown"
                if actual is None
            ...
            db.execute(
                "UPDATE reservations SET actual=?,state=?,response_id=? WHERE id=?",
    ...
    def report(self):
        with self.connect() as db:
            ...
            rows = [
                dict(r) for r in db.execute("SELECT * FROM reservations ORDER BY id")
            ]
        ...
        return {
            "limit": self.limit,
            "known_cost": known,
            "held_for_unknown": held,
            "available": self.limit - known - held,
            "reservations": rows,
        }
...
class MeasuredClient:
    ...
    def create(self, **kwargs):
        if kwargs["model"] != MODEL_ID or not 1 <= kwargs["max_tokens"] <= 4000:
            raise RuntimeError(
                "этот бюджет рассчитан только для DeepSeek V4 Flash и выхода до 4000"
            )
        ...
        input_bound = sum(
            len(m["content"].encode("utf-8")) for m in kwargs["messages"]
        ) + 1024 * (len(kwargs["messages"]) + 1)
        if input_bound + kwargs["max_tokens"] > CONTEXT_LIMIT:
            raise RuntimeError("консервативная граница запроса превышает окно")
        reserve = (
            input_bound * PRICE_CAP["prompt"]
            + kwargs["max_tokens"] * PRICE_CAP["completion"]
        ) / 1_000_000
        # Служебный вызов узнаётся по началу его системного промпта. Договорённость
        # хрупкая, поэтому она проверяется офлайн-тестом: разъехавшись, она не
        # сломала бы ничего заметного — просто цена памяти растворилась бы в счёте
        # за обычные ответы, и Sticky Facts выглядела бы дешевле, чем есть.
        kind = (
            "facts"
            if kwargs["messages"][0]["content"].startswith(FACTS_KIND_MARKER)
            else "chat"
        )
        ...
        while True:
            try:
                ...
                if time.monotonic() >= deadline:
                    raise RuntimeError(
                        "не дождались освобождения резерва; запрос не отправлен"
                    )
                time.sleep(1)
        kwargs["extra_body"] = {
            "provider": PROVIDER,
        ...
        if usage and (
            (getattr(usage, "prompt_tokens", None) or 0) > input_bound
            or (getattr(usage, "completion_tokens", None) or 0) > kwargs["max_tokens"]
        ):
            with self.ledger.connect() as db:
                db.execute(
                    "UPDATE reservations SET state='overrun' WHERE id=?", (ticket,)
                )
