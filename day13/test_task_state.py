...
def reach(agent, stage: Stage) -> TaskState:
    ...
    if stage is Stage.EXECUTION:
        return state
    state = agent.apply_task("submit_result", "Данные собраны, выводы написаны")
...
def test_pause_every_stage(db):
    ...
    print("ok  pause/resume сохраняет точку на planning, execution, validation и done")
...
def test_restart_without_explanation(db):
    ...
    first.start_task("Сделать миграцию каталога без простоя")
...
def test_schema_v2_upgrade(directory):
    ...
    print("ok  база схемы v2 открывается и получает таблицы FSM/workflow v4")
