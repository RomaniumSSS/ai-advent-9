"""Тест логики разбора ответа. Работает без обращения к API, поэтому быстро и бесплатно.

Проверяются не ответы модели, а наши выводы о них. Легко написать проверку, которая
хвалит не тот рычаг. Главный случай: ответ, обрезанный по max_tokens. Стоп-слова в нём
нет, и наивная проверка запишет это в заслугу stop.
