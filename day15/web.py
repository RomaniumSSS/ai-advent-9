"""Локальная панель дня 15: контролируемые переходы задачи."""
...
class Runtime:
    def __init__(
        ...
        *,
        ...
        self._seed_invariants(self.agent)

    @staticmethod
    def _seed_invariants(agent: Agent) -> None:
        for invariant in DEMO_INVARIANTS:
            agent.store.save_invariant(invariant)
    ...
    def switch_scope(self, session: str, task: str, user: str) -> None:
        ...
        self._seed_invariants(candidate)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _dispatch(self, path: str, body: dict) -> dict:
        ...
        if path == "/api/demo-user":
            ...
            self.runtime._seed_invariants(candidate)
    ...
    def _chat(self, text: str) -> dict:
        ...
        model_called = (
            ...
            else reply.finish_reason not in {"local_state", "invariant_refusal", "invariant_explanation", "transition_refusal"} and not reply.overflow
        )
        state_error = None
        try:
        ...
        return {
            ...
            "saved": reply.saved,
            "decision": (
                "refuse" if reply.finish_reason in {"invariant_refusal", "invariant_output_blocked", "transition_refusal"}
                else "explain" if reply.finish_reason == "invariant_explanation" else "allow"
            ),
            "usage": {
