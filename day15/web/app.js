...
function renderTask(task, events, workflow, denials) {
  renderPipeline(task);
  ...
  if (task) {
    ...
    $("task-status").textContent = task.stage === "done" ? "done" : task.status;
    ...
    $("pause").hidden = paused || task.stage === "done";
    $("resume").hidden = !paused;
    $("advance").hidden = paused || task.stage === "done";
    $("try-skip").hidden = paused || task.stage === "done";
    ...
    $("advance").textContent = `Попробовать ${task.expected_action || "переход"}`;
  }
  ...
  const rejected = denials.length ? denials.map((item) => {
    const row = document.createElement("div");
    row.className = "event";
    row.textContent = `v${item.version} · ${item.stage} · ${item.action}: ${item.reason}`;
    return row;
  }) : [Object.assign(document.createElement("div"), {className: "empty", textContent: "Отказов пока нет"})];
  $("transition-denials").replaceChildren(...rejected);
}
...
function render(state) {
  ...
  renderTask(state.task_state, state.task_events || [], state.workflow || {}, state.transition_denials || []);
...
$("try-skip").onclick = () => run(async () => {
  const result = await api("task/action", {
    action: currentState?.task_state?.stage === "planning" ? "approve" : "submit_plan",
    result: "Пропустить этап",
  });
  render(result.state);
  status(result.message);
});
