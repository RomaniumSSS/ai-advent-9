"""День 7: панель с двумя агентами, помнящими разговор после перезапуска.
...
Одна база с терминалом — намеренно. `uv run day07/cli.py --session left` и левая
панель это один и тот же разговор: начали в терминале, продолжили в браузере.

Запуск:
    uv run day07/web.py
    uv run day07/web.py --port 9000 --left работа --right черновик
"""
...
def build_panels(db_path, sessions: dict[str, str]) -> None:
...
def main() -> None:
    ...
    build_panels(args.db, {"left": args.left, "right": args.right})
