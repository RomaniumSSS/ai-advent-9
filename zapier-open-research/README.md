...
Если задан поисковый запрос, API просматривает одну страницу из 100 результатов OpenAlex,
отсекает заголовки с title relevance ниже 20/40 и отдаёт не более заданного размера
poll. Это эвристика для отсечения случайных full-text совпадений, не смысловая
экспертиза статьи. OpenAlex иногда возвращает разные ID для похожих записей;
совпадения по названию пока не объединяются.
...
`Record Review Score` — отдельный Zapier action для проверки replay-safe записи в
демонстрационный ledger. Его +10 баллов за webhook не меняют автоматический рейтинг
статьи и не означают человеческую рецензию.
...
Скриншоты опубликованных Zap и фактических запусков находятся в
[screenshots/](screenshots/); состояние ручной проверки — в [VERIFICATION.md](VERIFICATION.md).
