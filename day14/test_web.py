...
def test_validation_loop(runtime):
    ...
    dispatch(runtime, "/api/action", {"result": "План"})
    dispatch(runtime, "/api/action", {"result": "Результат"})
    ...
    dispatch(runtime, "/api/action", {"result": "Исправленный результат"})
    dispatch(runtime, "/api/action", {"result": "Принято"})
