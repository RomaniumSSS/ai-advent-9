"""CLI дня 14: накопительный агент, Task State Machine и инварианты."""
...
def command(agent: Agent, line: str) -> bool:
    if line == "/state":
        ...
        print(json.dumps([item.to_dict() for item in agent.store.load_invariants()], ensure_ascii=False, indent=2))
    elif line == "/checks":
        print(json.dumps(agent.store.invariant_checks(), ensure_ascii=False, indent=2))
    elif line == "/help":
        print("/start ЦЕЛЬ · /state · /act СОБЫТИЕ РЕЗУЛЬТАТ")
        print("/pause · /resume · /events · /invariants · /checks · /clear · /exit")
    else:
        return False
    return True
